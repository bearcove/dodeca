...
| `lsp-014` | `doing` | Frontmatter document links | Template, asset, and data-file fields in frontmatter become navigable. | Template fields are resolved through the authoring model; asset/data fields need typed path-bearing frontmatter fields before they can be linked without guessing from arbitrary `extra` keys. |
| `lsp-015` | `doing` | Template navigation | Go to definition for template extends/includes, blocks, macros, filters, and tests. | Path-bearing `extends`, `include`, and `import` targets resolve through Gingembre parsing and the authoring template model; block/macro/filter/test targets remain. |
