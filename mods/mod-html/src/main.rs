...
fn metadata_to_json(meta: &CodeExecutionMetadata) -> String {
    let deps_json: Vec<String> = meta.dependencies.iter().map(|d| {
        let source = match &d.source {
            mod_html_proto::DependencySource::CratesIo => "crates.io".to_string(),
            mod_html_proto::DependencySource::Git { url, commit } => format!("git:{}@{}", url, &commit[..7.min(commit.len())]),
            mod_html_proto::DependencySource::Path { path } => format!("path:{}", path),
        };
...
dodeca_plugin_runtime::run_plugin!(HtmlProcessorImpl);
