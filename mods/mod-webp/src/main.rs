...
use mod_webp_proto::{WebPProcessor, WebPResult, WebPEncodeInput, WebPProcessorServer};
...
/// Service wrapper for WebPProcessor to satisfy ServiceDispatch
struct WebPProcessorService(Arc<WebPProcessorServer<WebPProcessorImpl>>);

impl ServiceDispatch for WebPProcessorService {
    fn dispatch(
        &self,
        method_id: u32,
        payload: &[u8],
...
const SHM_CONFIG: ShmSessionConfig = ShmSessionConfig {
    ...
    slot_size: 65536,   // 64KB per slot (fits most WebP data)
...
async fn main() -> Result<()> {
    ...
    let dispatcher = dispatcher.add_service(WebPProcessorService(Arc::new(WebPProcessorServer::new(
        WebPProcessorImpl,
    ))));
    ...
    tracing::info!("WebP plugin ready, waiting for requests");
