...
/// Service wrapper for SvgoOptimizer to satisfy ServiceDispatch
struct SvgoOptimizerService(Arc<SvgoOptimizerServer<SvgoOptimizerImpl>>);

impl ServiceDispatch for SvgoOptimizerService {
    fn dispatch(
        &self,
        method_id: u32,
        payload: &[u8],
...
const SHM_CONFIG: ShmSessionConfig = ShmSessionConfig {
    ...
    slot_size: 65536,   // 64KB per slot (fits most SVG files)
...
async fn main() -> Result<()> {
    ...
    let dispatcher = dispatcher.add_service(SvgoOptimizerService(Arc::new(SvgoOptimizerServer::new(
        SvgoOptimizerImpl,
    ))));
    ...
    tracing::info!("SVGO plugin ready, waiting for requests");
