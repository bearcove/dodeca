...
/// Service wrapper for SearchIndexer to satisfy ServiceDispatch
struct SearchIndexerService(Arc<SearchIndexerServer<SearchIndexerImpl>>);

impl ServiceDispatch for SearchIndexerService {
    fn dispatch(
        &self,
        method_id: u32,
        payload: &[u8],
...
const SHM_CONFIG: ShmSessionConfig = ShmSessionConfig {
    ...
    slot_size: 65536,   // 64KB per slot (fits most search data)
...
async fn main() -> Result<()> {
    ...
    let dispatcher = dispatcher.add_service(SearchIndexerService(Arc::new(SearchIndexerServer::new(
        SearchIndexerImpl,
    ))));
    ...
    tracing::info!("Pagefind plugin ready, waiting for requests");
