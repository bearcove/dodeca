...
/// Service wrapper for Minifier to satisfy ServiceDispatch
struct MinifierService(Arc<MinifierServer<MinifierImpl>>);

impl ServiceDispatch for MinifierService {
    fn dispatch(
        &self,
        method_id: u32,
        payload: &[u8],
...
async fn main() -> Result<()> {
    ...
    let dispatcher = dispatcher.add_service(MinifierService(Arc::new(MinifierServer::new(
        MinifierImpl,
    ))));
    ...
    tracing::info!("Minify plugin ready, waiting for requests");
