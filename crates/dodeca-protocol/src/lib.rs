...
pub struct AnnotateReq {
    ...
    /// Idempotency key generated by the overlay, one per logical save and reused
    /// across its retry-once on a lost ack. The server dedupes on it: a new note
    /// adopts the nonce as its thread id; a reply stores it in the note's `nonce`
    /// field. A repeated write returns the existing note instead of inserting a
    /// duplicate.
    pub nonce: String,
}
