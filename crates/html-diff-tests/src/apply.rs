    ...
    match patch {
        ...
        Patch::ReplaceInnerHtml { path, html } => {
            ...
            match node {
                ...
                Node::Text(_) => {
                    return Err("ReplaceInnerHtml: cannot replace innerHTML of text node".to_string());
                }
            }
        }
...
mod tests {
    ...
    fn test_apply_remove() {
        let mut node =
            Node::parse("<html><body><p>First</p><p>Second</p></body></html>").unwrap();
        apply_patches(&mut node, &[Patch::Remove { path: NodePath(vec![1]) }]).unwrap();
