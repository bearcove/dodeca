...
/// Dump the Styx schema for DodecaConfig to stdout.
/// This enables Styx tooling to discover the config schema via CLI.
/// See: https://styx.bearcove.eu/tools/schema-distribution/
fn dump_styx_schema() {
    use config::DodecaConfig;

    // Get crate version from Cargo.toml
    let version = env!("CARGO_PKG_VERSION");

    // Generate schema from DodecaConfig type
    let schema = styx_schema::generate::to_styx_schema::<DodecaConfig>();
...
fn main() -> Result<()> {
    // Handle @dump-styx-schema before anything else (for Styx tooling integration)
