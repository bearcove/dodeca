...
use super::lexer::is_valid_req_char;
use super::positions::{ByteOffset, LineNumber, RefLocation};
use super::{RuleId, parse_rule_id};
...
pub fn extract(path: &Path, source: &str) -> CodeUnits {
    ...
    match super::languages::for_ext(ext) {
        Some(lang) => (lang.extract)(path, source),
        None => CodeUnits::new(),
    }
}

/// No-op code-unit extractor for config/data languages (YAML, JSON5).
///
/// These formats carry requirement references in comments — extracted via the
/// registered grammar in [`extract_refs_with_warnings`] — but have no functions
/// or types to surface as structural code units, so reverse coverage attributes
/// their references to no enclosing unit.
pub fn extract_config(_path: &Path, _source: &str) -> CodeUnits {
    CodeUnits::new()
}
...
pub fn extract_refs_with_warnings(path: &Path, source: &str) -> ExtractedRefs {
    ...
    let Some(lang) = super::languages::for_ext(ext) else {
    ...
    let file_code_mask = super::markdown::compute_doc_comment_code_mask(source);
...
fn extract_full_refs_from_text(
    ...
    while let Some((start_idx, ch)) = chars.next() {
        ...
        if super::markdown::is_code_index(start_idx, &code_mask)
            || super::markdown::is_code_index(file_idx, file_code_mask)
        {
            prev_ch = Some(ch);
            continue;
        }
...
fn extract_full_relation_annotations_from_text(
    ...
    while let Some(rel) = text[search_start..].find("@relation") {
        ...
        if super::markdown::is_code_index(hit_start, code_mask)
            || super::markdown::is_code_index(base_offset.as_usize() + hit_start, file_code_mask)
        {
...
fn try_parse_req_ref(
    ...
    let Some(ParsedFullRef::Parsed {
        verb: _,
        req_id,
        end_idx: _,
    }) = try_parse_full_ref(chars)
    else {
        return None;
    };
    Some(req_id)
